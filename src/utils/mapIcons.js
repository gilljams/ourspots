...
export const createUserIcon = (heading = null) => {
  ...
  const coneHtml = heading !== null ? `
    <div style="
      ...
      background: conic-gradient(
        from ${heading - 35}deg,
        transparent 0deg,
        rgba(59, 130, 246, 0.05) 5deg,
        rgba(59, 130, 246, 0.35) 35deg,
        rgba(59, 130, 246, 0.05) 65deg,
        transparent 70deg
      );
      -webkit-mask-image: radial-gradient(circle, transparent 10%, black 14%, black 40%, transparent 70%);
      mask-image: radial-gradient(circle, transparent 10%, black 14%, black 40%, transparent 70%);
      pointer-events: none;
      z-index: 0;
    "></div>
  ` : '';
  ...
  return L.divIcon({
    html: `<div class="user-position-marker" style="position: relative; width: 20px; height: 20px; overflow: visible;">
      ${coneHtml}
